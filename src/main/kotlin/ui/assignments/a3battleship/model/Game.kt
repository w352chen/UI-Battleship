...
class Game (val dimension: Int, val debug: Boolean = false) {
    ...
    val humanReadyToStartAttack = ReadOnlyBooleanWrapper(false)
//    val humanReadyToStartAttack : ObservableBooleanValue = SimpleBooleanProperty(false)
    ...
    fun removeShip(shipId: Int) {
        println("removeShip: shipId = $shipId")
        if ((gameState.value == GameState.HumanSetup) or (gameState.value == GameState.AiSetup)) {
    ...
    fun getCell(player: Player, x: Int, y: Int) : Cell {
        return boards[player]!!.getCell(x, y)
    }
